...
try {
  const api = {
    ...
    db: {
      ...
      getTopAppsByCategory: (category: string, limit?: number) => ipcRenderer.invoke(CH.db.getTopAppsByCategory, category, limit),
      getTopTitlesByCategory: (category: string, limit?: number) => ipcRenderer.invoke(CH.db.getTopTitlesByCategory, category, limit),
